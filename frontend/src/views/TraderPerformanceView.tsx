...
export const TraderPerformanceView: React.FC<TraderPerformanceViewProps> = ({ user, currentUser, onBack, onAskAI }) => {
  ...
  return (
    <div className="w-full max-w-md mx-auto pb-24 px-3 sm:px-0 space-y-3">
      ...
      <section className="bg-[#07130c] border border-emerald-500/30 rounded-2xl p-4"><div className="flex items-center gap-3"><img src={profile.avatar} alt={profile.username} className="w-11 h-11 rounded-full object-cover" /><div><h1 className="text-base font-black text-white">Performance Trader Detail</h1><p className="text-xs text-neutral-400">@{profile.username}</p></div></div><div className="mt-4"><span className="text-[11px] text-neutral-400">Cumulative Profit</span><strong className={`block text-3xl font-black font-mono ${(summary.cumulativeProfit || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{money(summary.cumulativeProfit || 0)}</strong></div><button onClick={() => onAskAI(data)} disabled={!currentUser} className="mt-4 w-full py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 disabled:opacity-40 text-white text-xs font-bold flex items-center justify-center gap-2"><BrainCircuit className="w-4 h-4" /> Tanya AI</button></section>
